...
const navLinks = [
  ...
  { href: '#academic-progress', label: 'Academics', id: 'academic-progress' },
];

const homepageSectionIds = ['home', 'about', 'projects', 'academic-background', 'blogs', 'academic-progress', 'contact'];
            ...
            <nav className={cn(
              ...
              <div className="hidden md:flex items-center gap-1 text-sm">
                {navLinks.map((link) => {
                    ...
                    // Special check for academics link
                    const isAcademicsActive = (link.id === 'academic-progress' && pathname === '/programs') || (link.id === 'academic-progress' && activeSection === 'academic-progress');
            ...
            <motion.div className="flex h-full flex-col items-center justify-center gap-8 text-center">
              {navLinks.map((link) => {
                const isActive = activeSection === (link.id || link.href);
                 const isAcademicsActive = (link.id === 'academic-progress' && pathname === '/programs') || (link.id === 'academic-progress' && activeSection === 'academic-progress');
                return (
                  <motion.div key={`${link.href}-${link.label}`} variants={mobileLinkVariants}>
                    <Link
                      href={getLinkHref(link.href)}
                      className={cn(
