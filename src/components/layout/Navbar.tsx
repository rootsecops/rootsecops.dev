...
const homepageSectionIds = ['home', 'about', 'projects', 'experience', 'blogs', 'academic-progress', 'contact'];
        ...
        >
          <div className="max-w-5xl mx-auto">
            <nav className={cn(
              ...
              <Link href="/" className="flex items-center pl-3" onClick={handleLogoClick}>
                  <AnimatedLogo />
              </Link>
              ...
              <div className="hidden md:flex items-center gap-1 text-sm">
                {navLinks.map((link) => {
                    const isActiveOnHomepage = activeSection === (link.id || link.href);
                    // Special check for academics link on non-homepage
                    const isActiveOnPage = pathname.startsWith(link.href) && link.href !== '/';
                    const isAcademicsActive = (isHomepage && activeSection === 'academic-progress') || pathname.startsWith('/academics');

                    const isActive = link.id === 'academics' ? isAcademicsActive : (isActiveOnHomepage || isActiveOnPage);
                    
                    return (
                        <motion.div key={`${link.href}-${link.label}-desktop`} className="relative" whileHover={{ y: -2 }}>
                            <Link
          ...
          >
            <motion.div className="flex h-full flex-col items-center justify-center gap-8 text-center">
              {navLinks.map((link) => {
                ...
                return (
                  <motion.div key={`${link.href}-${link.label}-mobile`} variants={mobileLinkVariants}>
                    <Link
                      href={getLinkHref(link.href)}
                      className={cn(
              ...
              <motion.div key="contact-mobile" variants={mobileLinkVariants}>
                  <Link
