...
import { Github, ExternalLink, ArrowRight, BookOpen, Loader2 } from 'lucide-react';
...
export default function ProjectsSection({ isHomePage = false, searchTerm = '' }: { isHomePage?: boolean, searchTerm?: string }) {
  ...
  }, [allProjects, searchTerm, isHomePage]);

  // If this is the projects page, and we are on mobile, and there is a search term,
  // we don't render the main list. The SearchDialog will render its children.
  if (!isHomePage && isMobile && searchTerm) {
      return null;
  }
      ...
      <div className="container mx-auto px-4">
        ...
        {isLoading ? (
          ...
          <>
                ...
                >
                  <Card className="glass-card flex flex-col overflow-hidden group h-full">
                    ...
                    <CardContent className="flex-grow">
                      <ReadMore text={project.description} className="text-sm text-muted-foreground mb-3 text-balance" />
            ...
            {!isLoading && filteredProjects.length === 0 && !isHomePage && (
              <div className="text-center min-h-[200px] flex flex-col justify-center items-center">
                ...
                <p className="text-muted-foreground mt-2">Your search for "{searchTerm}" did not match any projects.</p>
              </div>
            )}
          </>
        )}
